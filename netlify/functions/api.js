...
exports.handler = async function(event) {
  ...
  if (action === 'generate_desc') {
    ...
    if (!anthropicKey) return res(headers, 500, { error: 'Anthropic API key not configured' });

    const stockMap = { available: 'متوفر', preorder: 'حجز مسبق', soon: 'قريباً', out: 'غير متوفر' };

    const systemPrompt = `You MUST use the web_search tool to search for this exact product on redseafish.com before writing any description.
...
If you find the product, write a 3-4 sentence Arabic marketing description based only on what you found on redseafish.com.`;
    ...
    const userContent = `Product: "${name}"
...
Search redseafish.com for this product now. If found, write a 3-4 sentence Arabic description. If not found, respond: NOT_FOUND`;
    ...
    try {
      ...
      /* agentic loop — handles tool_use stop_reason (server-side search completes per turn) */
      let loops = 0;
      ...
      if (!text || /NOT_FOUND/i.test(text)) {
        return res(headers, 200, { desc: null, notFound: true });
      }

      return res(headers, 200, { desc: text });
    } catch (e) {
      console.error('generate_desc:', e.message);
      return res(headers, 502, { error: 'API error: ' + e.message });
    }
  }
