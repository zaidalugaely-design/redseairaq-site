...
## الهوية البصرية — النظام الموحّد (فرع `redesign-dark`، قيد المعاينة، لم يُدمج مع main بعد)

**تحديث مهم:** بعد جلسة "إعادة بناء طبقة التنسيق"، لوحة الإدارة **لم تعد معزولة** — توحّدت مع نفس النظام الداكن أدناه (بلا استثناء تصميمي، فقط وظيفتها ووضوحها مضمونان). إلى أن يحصل الدمج مع `main`، هذا كله موجود فقط على فرع `redesign-dark`؛ `main` نفسه ما زال بالنظام الفاتح القديم بلا أي تغيير.
...
- **معرّفات الفئات تأتي من Supabase دائماً — لا تُكتب يدوياً أبداً.** أي مصفوفة/كائن بالكود يحتاج معرّف فئة (`category.id`) يجب أن يُبنى من `CATS` المُحمَّلة فعلياً (أو يُتحقَّق أعضاؤه منها في وقت التشغيل)، وليس بنسخ أسماء تبدو منطقية يدوياً. هذا الخلل بالذات ضرب `homeCats` (أُصلح — انظر أدناه)، وما زال موجوداً بـ3 مواضع أخرى لم تُصلَح بعد لأنها خارج نطاق أي أمر طُلب حتى الآن:
  - `let CATS=[...]` (~2485): القيمة الاحتياطية لمتغيّر `CATS` قبل اكتمال المزامنة أو إن فشلت — أخطرها لأنها تعني فئات بمعرّفات وهمية للزبون بالكامل لو انقطعت الشبكة.
  - `const SUGG_MAP` (~2517)، تُستخدم بصفحة المتجر وصفحة تفاصيل المنتج: خريطة "فئات ذات صلة" بمعرّفات قديمة — 7 من 13 فئة حقيقية لا تُظهر أي اقتراح بسببها.
  - `DEFAULT_CAT_IDS` (~5679) و`saveCustomCats()` (~2511، نسخة أنقص حتى من الأولى): تصنيف "افتراضي مقابل مخصّص" بلوحة إدارة الفئات بالأدمن — تصنيف معلوماتي خاطئ لـ7 فئات، لا يمنع أي وظيفة.
