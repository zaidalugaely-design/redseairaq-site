...
- **`products` (متغير)**: يُهيَّأ من كاش `localStorage['rsi_shop_products']` أولاً (إن كان صالحاً)، تُحدَّث بعد sync — انظر "بيانات قديمة معروضة للزبون" أدناه لتفاصيل الكاش
...
- `cacheGet`/`cacheSet`: مفتاحان ثابتان **`rsi_shop_products`/`rsi_shop_categories`** (بدل `rsi_prods`/`rsi_cats` القديمين) — بصيغة `{v, data, ts}`. المفتاح الجديد كلياً يُبطل تلقائياً أي بيانات قديمة لدى أي زائر حالي فور أول تحميل بعد هذا النشر، بلا حاجة لمسح يدوي (`CACHE_VERSION` يُرفع فقط عند تغيير حقيقي بشكل الكائن المخزَّن مستقبلاً).
...
- فشل الجلب: إن وُجد كاش سابق صالح، يبقى معروضاً **مع** شريط تحذير `#dataStatusBar` وزر إعادة محاولة — لا يُخفى الإشعار أبداً، ولا يُعطَّل الشراء (السعر وزر «أضف للسلة» يبقيان ظاهرين دائماً). بلا أي كاش على الإطلاق: `shopEmptyStateHtml()` الموجودة أصلاً (حالة فارغة صادقة + زر إعادة محاولة).
- أثناء الجلب: `#dataStatusBar` (`data-status-bar loading`) شريط "جاري تحديث الأسعار والمنتجات..." مترجم للغات الثلاث.
- `renderCurrentRoute()` يُستدعى دائماً في نهاية `syncFromSupabase()` — نجح الجلب أو فشل.

**آليتان جُرِّبتا هنا (جلسة "بيانات طازجة دائماً") ثم أُزيلتا لاحقاً (أغسطس 2026، جلسة "حذف الإجراءات غير المجدية") — لا تُعادا:**
- **حاجز الساعة (`STALE_MS`/`dataDegraded`):** كان يُخفي السعر ويستبدل زر «أضف للسلة» بزر واتساب حين يفشل الجلب ويتجاوز عمر الكاش ساعة. **لم يعالج السبب الجذري** (فشل جلب صامت — مُصلَح فعلاً بفحص `Array.isArray` + شريط التحذير أعلاه) وكان يوقف البيع عند أي تعثّر شبكة عابر لدى الزبون. حُذف بالكامل (`STALE_MS`، `dataDegraded`، الحاجز داخل `addToCart()`، `askPriceUnknown()`).
- **سطر تشخيص المزامنة (`#syncDiag`):** سطر خافت أسفل كل صفحة يعرض حالة آخر مزامنة/عمر البيانات/معرّف البناء — كان ظاهراً لكل زبون على الموقع الحي، غير احترافي. حُذف (`#syncDiag`، `renderSyncDiag()`، `lastSyncError`).
...
- **داخل صفحة منتج مفتوحة:** `patchProductDetailPrice(id)` تُحدِّث نص عنصر `#dpr-{id}` (والـ`<option>` لكل variant إن وُجد) في مكانه مباشرة — لا `renderCurrentRoute()` كامل، لا نقل الزائر عن الصفحة، لا إعادة تحميل الصورة/الوصف.
- فشل الجلب الصامت: `handleSilentRevalidateFailure()` تعرض شريط `#dataStatusBar` (تحذير فقط، بلا تأثير على السعر/زر الشراء — انظر "آليتان جُرِّبتا... ثم أُزيلتا" أعلاه) — بلا مؤشر تحميل مطلقاً.
...
- `let CATS=[...]` (12 فئة بمعرّفات قديمة غير مطابقة إطلاقاً لمعرّفات Supabase الحقيقية) و`const DEF_PRODUCTS=[...]` (8 منتجات وهمية) **حُذفتا نهائياً**. الآن: `CATS`/`products` تُهيَّآن من كاش `rsi_shop_categories`/`rsi_shop_products` (`cacheGet()`) إن كان صالحاً وإلا `[]` — لا قيمة احتياطية مكتوبة يدوياً بعد الآن.
