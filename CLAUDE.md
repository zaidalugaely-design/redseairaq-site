...
- `renderHome()` فيها مصفوفة `homeCats` (لالتقاط منتج ممثّل واحد لكل فئة بالصفحة الرئيسية) لا تزال تستخدم معرّفات فئات قديمة (`tanks`, `salt`, `filtration`, ...) غير مطابقة لمعرّفات Supabase الحقيقية (`cat_tanks`, ...)، فتفشل بصمت وتقع على حلقة fallback عامة. خلل موجود فعلاً لكن خارج نطاق أي دفعة طُلبت — لم يُصلَح، مذكور هنا فقط كملاحظة لمهمة مستقبلية.
