...
- أيقونات الفئات إيموجي مخزّنة كنص في `categories.icon` بقاعدة بيانات Supabase (📦🧂🧪🌊🔬⚗️📏🧬⚡💡🐠) — ليست في الكود، ولا تُعدَّل قاعدة البيانات من هذا العمل. الكود فيه مجموعة `CAT_ICONS` بأيقونات SVG خطية جاهزة لكنها غير مربوطة بمعرّفات الفئات الحقيقية — ربطها بطبقة العرض (بدون لمس القاعدة) مهمة منفصلة موثّقة كـ commit مستقل.
