...
**التشخيص الفعلي (أغسطس 2026):** الكود كان أصلاً يجلب من Supabase مع `cache:'no-store'` في كل تحميل كامل للصفحة (لا كاش HTTP على مستوى الطلب)، ووسوم `<meta http-equiv="Cache-Control">` موجودة في `<head>` (غير موثوقة فعلياً في المتصفحات الحديثة لكنها موجودة). **السبب الحقيقي المرجّح: تبويب متصفح مفتوح من قبل** — الـ SPA تجلب من Supabase **مرة واحدة فقط عند فتح الصفحة** (`boot()`)، فتبويب بقي مفتوحاً منذ قبل تعديل سعر بالأدمن يستمر بعرض السعر القديم من الذاكرة إلى الأبد حتى يُعاد تحميله يدوياً — بينما تبويب خفي جديد يجلب فوراً السعر الحالي. إضافة لذلك: عند فشل الجلب (انقطاع شبكة)، الكود القديم كان يستمر بعرض الكاش المحلي **بصمت تام بلا أي إشعار** للزبون بأن البيانات قد لا تكون محدّثة.

**الحل الجذري — طبقتان منفصلتان:**
...
- `cacheGet`/`cacheSet` (`CACHE_VERSION` ثابت): كاش `rsi_prods`/`rsi_cats` أصبح بصيغة `{v, data, ts}` — أي تغيير مستقبلي في بنية البيانات يتطلب رفع `CACHE_VERSION` ليُبطَل الكاش القديم تلقائياً بدل أن يُقرأ بشكل خاطئ.
...
- فشل الجلب (لا اتصال): إن وُجد كاش سابق صالح، يبقى معروضاً **مع** شريط تحذير `#dataStatusBar` (`data-status-bar stale`) وزر إعادة محاولة — لا يُخفى الإشعار أبداً. بلا أي كاش على الإطلاق: `shopEmptyStateHtml()` الموجودة أصلاً (حالة فارغة صادقة + زر إعادة محاولة).
- أثناء الجلب: `#dataStatusBar` (`data-status-bar loading`) شريط "جاري تحديث الأسعار والمنتجات..." مترجم للغات الثلاث.

**(ب) تجميد سعر السلة (`addToCart`/`renderCart`/`doOrder`, `index.html` + `save_order`, `api.js`):**
