...
- **النسخة الاحتياطية قبل أي تعديل جماعي على `products`:** `docs/products-backup-<YYYY-MM-DD>.json` (نسخة كاملة JSON لكل الأعمدة، كل الصفوف). أُخذت أول نسخة بتاريخ 2026-08-11 قبل إصلاح الأوصاف. خذ نسخة جديدة بنفس النمط قبل أي تعديل جماعي مستقبلي إن لم تكن هناك نسخة أحدث.

### تسمية منتجات المكمّلات المتشابهة (عسرة/كالسيوم/مغنيسيوم)

كانت 5 منتجات مكمّلات بأسماء متضاربة يصعب تمييزها على الزبون (خلط عربي/إنجليزي بلا نمط، معلومة التمييز الوحيدة مدفونة وسط الاسم). وُحّدت بنمط: **`{العنصر الكيميائي بالعربي} ({الرمز الكيميائي}) — {وصف تفريقي إن وُجد} {الحجم}`**

| قبل | بعد |
|---|---|
| `Kh red sea 500ml عسره` | `عسرة (KH) — Red Sea 500ml` |
| `عسره لتحفيز نمو الكارولينا 500ml kh` | `عسرة (KH) — لتحفيز نمو الكارولينا 500ml` |
| `Mg مغنيسيوم 500ml` | `مغنيسيوم (Mg) 500ml` |
| `كالسيوم سائل ٥٠٠ مل` | `كالسيوم (Ca) سائل 500ml` |
| `سيت عسره كالسيوم مغنيسيوم نصل لتر للبطل الواحد` | `طقم (KH + Ca + Mg) — عسرة وكالسيوم ومغنيسيوم، 500ml لكل عبوة` |

طبّق هذا النمط على أي منتج جديد/مُعاد تسميته من نفس عائلة المكمّلات الأساسية (كالسيوم/عسرة/مغنيسيوم) لإبقاء الفرق بين المنتجات واضحاً على الزبون.
