...
**كل الأماكن التالية بـ`index.html` كانت تحوي معرّفات الفئات الـ13 القديمة مكتوبة يدوياً، واستُبدلت بالأربع الجديدة فقط:** `CAT_ICONS` (أيقونة واحدة ممثّلة لكل فئة مدمجة، بدل أيقونة مستقلة لكل فئة قديمة)، `saveCustomCats()`، `SUGG_MAP`، `homeCats` بـ`renderHome()`، `caut` بـ`renderProductDetail()` (شارة "استخدم بحذر" — أصبحت تغطي فئتي `water_treatment`/`feeding_supplements` كاملتين بدل ثلاث فئات قديمة محددة، توسيع مقصود ناتج عن الدمج)، `DEFAULT_CAT_IDS`، ونص لوحة إدارة الفئات بالأدمن (`renderCatManagerList`)، بالإضافة لنص `categories_desc` بالثلاث لغات.

**صفحة PRODIBIO (`renderProdibio()`, مسار `#/prodibio`) كانت تعتمد على `category=eq.probidio` مباشرة بطلب Supabase — نُقلت السبعة منتجات التي كانت بفئة `probidio` (والتي دُمجت الآن ضمن `water_treatment` مع منتجات أخرى غير تابعة لـPRODIBIO) إلى استعلام `id=in.(...)` صريح بنفس السبعة معرّفات، حتى تستمر الصفحة بعرض نفس منتجات PRODIBIO بالضبط دون أي تغيير بالسلوك.** هذا يعني أن أي منتج PRODIBIO جديد يضيفه الأدمن مستقبلاً (حتى لو أُسنِد لفئة `water_treatment`) **لن** يظهر تلقائياً بصفحة PRODIBIO — يحتاج إضافة معرّفه يدوياً لقائمة `id=in.(...)` بدالة `renderProdibio()`. هذا قيد معروف ناتج عن حذف فئة `probidio` المستقلة، وليس خللاً.
